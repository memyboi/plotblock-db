...
module.exports = {
  name: 'warn',
  description: 'Warn a user off of the server with a reason.',
    ...
    if (talkedRecently.has(message.author.id)) {
      ...
      if (reason == "") {
        member.send("You have been warned in `" + message.guild.name + "`.\nNo reason given by moderator.") .then(() => {
          message.reply("Successfully warned " + member.user.tag + ".\nReason given: ```" + reason + "```")
        }) .catch((err) => {
          message.reply("Warned successfully! However, The message was not delivered to the user.")
        })
      } else {
