...
module.exports = {
	...
	async execute(interaction, client) {interaction.deferReply({ephemeral: true})
		...
		if (!reason) {
			...
			interaction.editReply({content: "User has been warned. No reason given.", ephemeral: true})
		} else {
			user.send({content: "*You have been warned by `"+interaction.user.username+"` from Plot Block [LIFESTEAL]!*\nThe reason given is: \n```"+reason+"```\nPlease refrain from breaking the rules once more. If you feel you are not familiar with the rules, please re-read the rules."})
			interaction.editReply({content: "User has been warned. Reason has been given:\n```"+reason+"```", ephemeral: true})
		}
