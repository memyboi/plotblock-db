...
client.on('interactionCreate', async interaction => {
  if (interaction.isButton()) {
    if (interaction.customId == "acc") {
      ...
      const row = new ActionRowBuilder()
        .addComponents(text)
      const textemoji = new Discord.TextInputBuilder()
        .setLabel("Clan Emoji")
        .setPlaceholder('The emoji that your clan will use. eg. <:Diamond:1024783000153640991>, <:Pearl:1024783025214595082>')
        .setStyle(Discord.TextInputStyle.Short)
        .setCustomId("clanemoji")
      const rowemoji = new ActionRowBuilder()
        .addComponents(textemoji)
      const modal = new ModalBuilder()
        .setTitle("Create a clan.")
        .setCustomId("ClanCreateModal")
        .setComponents(row, rowemoji)
      await interaction.showModal(modal)
